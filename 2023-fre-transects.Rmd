...
Raincoast Conservation Foundation staff conducted biophysical assessments of recently expanding tidal marshes in July and August 2023 to inform the upcoming work of PhD student Daniel Stewart and MSc student Daniel Hennigar (Martin Conservation Decisions Lab, University of British Columbia). Two main research questions are explored:
...
make_profile <- function(transect_data) {
    ggplot(data = transect_data, aes(x = hd, y = elevation, label = desc)) +
        xlab("Horizontal Distance (m)") +
        ylab("Elevation (m)") +
        geom_line() +
        geom_line(aes(y = carex_height),
                  color = "green4", linetype = "dashed", alpha = 0.7) +
        geom_point(size = 2.5) +
        geom_point(aes(y = carex_height),
                   color = "green4", size = 2.5, alpha = 0.7) +
        geom_point(aes(y = soil_pos - 0.3), color = "black", size = 2.5, shape = 17) +
        geom_text(max.overlaps = Inf, hjust = -0.1, angle = 90, size = 3) +
        scale_x_continuous(breaks = 5 * (transect_data$hd %/% 5)) +
...
Surveyors sampled one transect located in the southwest corner of the marsh, targeting an area of recent marsh expansion. The transect ran southeast for approximately 46 meters beginning in sparsely vegetated mudflat and ending at the edge of riparian vegetation. Substrate along this transect was a silty-clay texture at all sample points except at the lowest elevation where it was silty-clay-loam. Rooting depths at this site ranged from 2.5 cm at the lowest elevation to 22 cm at the highest elevation. Pore water was generally too turbid to collect accurate salinity measurements, however, two stations where sampling was possible resulted in a salinity range of 0 - 1 + 1 % for this transect. Low marsh vegetation was dominated by *V. anagallis-aquatica* interspersed mainly with *S. emersum* and *Persicaria spp.*. *C. lyngbyei* was dominant in the intermediate zone of the transect with *A. stolonifera* and *Equisetum fluviatile* as subdominant species. Average *C. lyngbyei* height tended to increase moving up the transect from 38.4 cm to 125.4 cm. *P. arundinacea* was dominant in the high marsh zone near the back of the marsh. *L. salicaria* was notably absent from the lower marsh zones at this site and only found in small amounts in the higher marsh zones.

```{r fig6, fig.cap = "Annacis Channel Middle transect including vegetation communities. Soil profile locations indicated with black triangles."}
transects %>%
    filter(site == "annacis_channel_mid") %>%
    make_profile()
```
...
The lower marsh area was a very sparsely vegetated mudflat with one plot near the foreshore including no plants at all. The sparse vegetation was composed mainly of *E. fluviatile* and *L. salicaria* with trace amounts of *E. palustris* and *C. stagnalis*. This transect had a very sharp transition into the high marsh zone which was composed mainly of *C. lyngbyei* and *Typha latifolia*. Average sedge height of *C. lyngbyei* varied along the transect from 39.8 to 108.6 cm. Incidents of other species were also observed in the high marsh zone and included *Juncus oxymeris* and *A. stolonifera*. *C. lyngbyei* transitioned into *P. arundinacea* towards the upland side of the high marsh zone.

```{r fig8, fig.cap = "Queensborough East elevation profile with associated vegetation communities. Soil profile locations indicated with black triangles."}
transects %>%
    filter(site == "queensborough_east") %>%
    make_profile()
```
...
```{r fig10, fig.cap = "Queensborough Bridge transect 1 elevation profile with associated vegetation communities. Soil profile locations indicated with black triangles."}
transects %>%
    filter(site == "queensborough_bridge_t1") %>%
    make_profile()
```

```{r fig 11, fig.cap = "Queensborough Bridge transect 2 elevation profile with associated vegetation communities. Soil profile locations indicated with black triangles."}
transects %>%
    filter(site == "queensborough_bridge_t2") %>%
    make_profile()
```
...
Two transects were assessed in the eastern section of this marsh during low tide on August 3rd, 2023. The first ran northwest for approximately 30 meters from the sandy berm into the high marsh zone. Substrate texture along this transect was variable but overall had higher sand content than most other sites sampled. Rooting zones along this transect were relatively shallow (i.e., 0 - 9 cm) with the exception of the most upland soil core (20 cm). Pore water salinity along this transect ranged from 0 - 1 + 1 %. The lower extent of this transect had sparse incidents of *C. stagnalis*, *E. parvula*, and *Persicaria spp.*, whereas the upper reach of the transect was dominated by *L. salicaria*. 

The second transect was located further east and ran for approximately 40 meters from silty mudflat into the high marsh zone. Substrate along this transect started out with sandier texture, but transitioned into loam/silt-loam after the sixteen meter mark. Rooting zones were still shallow along the lower half of the transect (i.e., 0 - 5 cm) but were deeper throughout the latter half (i.e., 20 - 21 cm). Pore water salinity range did not differ from the first transect. The lower reach of transect two had instances of *C. stagnalis*, *E. palustris* and *Bidens cernua*. The upper reach of this transect was dominated by *C. lyngbyei* with subdominant species including *L. salicaria* and *A. stolonifera*. Average *C. lyngbyei* height along this transect generally increased with a range of 59.2 cm to 126.2 cm.

```{r fig12, fig.cap = "Mitchell Island West transect 1 elevation profile with associated vegetation communities. Soil profile locations indicated with black triangles." }
transects %>%
    filter(site == "mitchell_is_west_t1") %>%
    make_profile()
```

```{r fig13, fig.cap = "Mitchell Island West transect 2 elevation profile with associated vegetation communities. Soil profile locations indicated with black triangles."}
transects %>%
    filter(site == "mitchell_is_west_t2") %>%
    make_profile()
```
...
One transect was assessed at this site on August 3rd, 2023. The transect ran north from the low elevation mudflat into the high marsh zone for approximately 20 meters. Due to quick rising tide, only four soil cores were able to be assessed at this site. Texture of the sampled cores varied including sandy loam, silt-loam, and silty-clay-loam. Rooting depth also varied along the transect from a minimum of 0 cm to a maximum of 21 cm. Vegetation in the low marsh included instances of *Callitriche stagnalis* and *Persicaria spp.*. The higher marsh zone was primarily dominated by *Carex lyngbyei* with subdominant species including *T. angustifolia* and *A. stolonifera*. Average *C. lyngbyei* generally increased along the transect with mean heights ranging from 74 to 136 cm. 

```{r fig14, fig.cap = "Mitchell Island East elevation profile with associated vegetation communities. Soil profile locations indicated with black triangles."}
transects %>%
    filter(site == "mitchell_is_east") %>%
    make_profile()
```
...
Surveyors sampled at this site from approximately 11:00 - 16:00 on August 28th, 2023. The site was accessed by boat from the Vancouver Wooden Boat Society dock in New Westminster. Log booms were observed around the entire perimeter of the island and several old barges and pilings were observed along the western end of the site. Two transects were sampled at this site. The first transect ran northwest for approximately 74 meters from low lying mudflat into a patch of high marsh vegetation. The second transect was located approximately 1.2 kilometers upstream of transect one and ran northwest for approximately 60 meters. The lower reach of transect 1 was dominated by an assemblage of mudflat species including *Persicaria spp.* and *V. anagallis-aquatica*. The vegetation community transitioned to a *L. salicaria*-dominant high marsh. Substrate along the lower reach of this transect had sandy loam to sandy clay loam texture with no obvious rooting zone. These core samples were shorter in length due to a suspected sand layer that was not able to be penetrated with the coring device. Substrate along the upper reach of the transect shifted to a silt clay loam texture with increasing rooting zones ranging from 2.5 to 20 cm. Pore water salinity was only able to be estimated in the upper reach of the transect and was consistently 0 + 1%. The low marsh zone of transect two was similar to that of Transect 1, with slightly more *L. salicaria* at lower elevations. The upper reach of this transect similarly transitioned to a *L. salicaria*-dominant high marsh zone. Substrate along this transect ranged from sandy loam to loam in texture. The lower half of the transect had no obvious rooting zone while the upper reach had rooting depths ranging from 4 to 18 cm. Pore water salinity was not able to be estimated along this transect due to lack of pore water in the soil pits.

```{r fig19, fig.cap = "Sapperton Bar transect 1 elevation profile with associated vegetation communities. Soil profile locations indicated with black triangles."}
transects %>%
    filter(site == "sapperton_bar_t1") %>%
    make_profile()
```

```{r fig20, fig.cap = "Sapperton Bar transect 2 elevation profile with associated vegetation communities. Soil profile locations indicated with black triangles."}
transects %>%
    filter(site == "sapperton_bar_t2") %>%
    make_profile()
```

## South Arm Marshes West

South Arm Marshes West comprised two small islands within the South Arm Marshes Wildlife Management Area, immediately east of Westham Island in the south arm of the lower Fraser River. Access was via boat during an appropriate low tide window. Surveyors sampled two transects, one on each island, from approximately 8:00 - 15:00 on August 29th. Transect 1 transected a newly formed marsh island composed mainly of *Eleocharis palustris*, with sparse incidental occurrences of *Schoenoplectus tabernaemontanii*. Transect 2 transected an adjacent stable marsh island which was far more floristically diverse, including a well established *Carex lyngbyei*-dominant community. Another visible distinction between to the locations was that Transect 2 contained large swarms of flying insects, while Transect 1 did not. Surveyors suspect that this invertebrate activity was amplified by the day's summer rains.

<!-- lacking soil info! -->

```{r fig21, fig.cap = "South Arm Marshes West transect 1 elevation profile with associated vegetation communities. Soil profile locations indicated with black triangles."}
transects %>%
    filter(site == "south_arm_marshes_west_t1") %>%
    make_profile()
```

```{r fig 22, fig.cap = "South Arm Marshes West transect 2 elevation profile with associated vegetation communities. Soil profile locations indicated with black triangles."}
transects %>%
    filter(site == "south_arm_marshes_west_t2") %>%
    make_profile()
```

## Frenchie's Island

Frenchies Island is location in the centre of the South Arm Marshes Wildlife Management Area. This sheltered marsh island is managed in partnership with Ducks Unlimited Canada and includes a restored former private farm whose dyke has since been breached. Natural tidal marshes extend for several hundred meters beyond the breached dyke and these were the locatiosn selected for sampling. Transect 1 was located northeast of the Frenchies dyked area and Transect 2 to the south.

Transect 1 targeted an area of recent marsh expansion on the north east side of Frenchies Island. This transect bisected a large, vigorous stand of *S. tabernaemontani* before transitioning to a *C. lyngbyei*-dominant high marsh zone. Soil texture ranged from silt-loam to silt-clay-loam for all soil cores on this tranect. Salinity ranged from 2% - 5% (+/- 1%). Mottling was common and observed in most soil horizons.

Tranect 2 targeted an area of apparent marsh stability. The sparsely vegetated low marsh gave way quickly to dense *C. lyngbyei*-dominant high marsh. The upland portion of the transect bisected a large, dense stand of near *T. spp.* monoculture. The transect also encountered two tidal channels, visible in figure 24. Soil texture was consistent with transect 1, though pore water salinity was not available in most locations for this transect. Salinity was only available at the first station, measuring 3% (+/- 1%).

```{r fig23, fig.cap = "Frenchies Island Transect 1 elevation profile with associated vegetation communities. Soil profile locations indicated with black triangles."}
transects %>%
    filter(site == "frenchies_island_t1") %>%
    make_profile() +
    scale_y_continuous(breaks = seq(-1.5, 7, by = 0.5),
                       limits = c(-1.3, 7))
```


```{r fig24, fig.cap = "Frenchies Island Transect 2 elevation profile with associated vegetation communities. Soil profile locations indicated with black triangles."}
transects %>%
    filter(site == "frenchies_island_t2") %>%
    make_profile() +
    scale_y_continuous(breaks = seq(-1.5, 7, by = 0.5),
                       limits = c(-1.3, 7))
```
...
A complete floristic survey was completed at each location. The extent of this survey was determined on a site-by-site basis. Surveyors recorded all vascular plants within the surrounding tidal marsh of each transect, not searching beyond any transitions to an obviously distinct plant community (e.g., the riparian zone). All species observed are summarized by site in the following table.

```{r}

library(gt)

flora_raw <- readRDS("./data/floristic_surveys.RDS")

flora <- flora_raw %>%
    mutate(occurs = "✓") %>%
    spread("species", occurs, fill = NA) %>%
    select(-low_marsh_only)

flora_table <- gt(flora) %>%
...
The complete results from soil core samples taken along each transect are summarized below. Columns H1 - H4 represent depth and soil texture for soil horizon 1 through 4 respectively. Salinity is presented in percent.

```{r}

soil <- readRDS("./data/soil_cores.RDS")
