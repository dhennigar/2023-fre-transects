...
make_profile <- function(transect_data) {
    ggplot(data = transect_data, aes(x = hd, y = elevation, label = desc)) +
        ...
        ylab("Elevation (m)") +
        geom_line() +
        geom_line(aes(y = carex_height), color = "green4", linetype = "dashed") +
        geom_point(size = 2.5) +
        geom_point(aes(y = carex_height), color = "green4", size = 2.5) +
        geom_text_repel(max.overlaps = Inf, hjust = -0.1, angle = 90, size = 2.5) +
        geom_point(aes(y = soil_pos - 0.25), color = "orange", size = 2.5, shape = 17) +
        ylim(-1.25, 7.5) +
        theme_minimal()
}
```
